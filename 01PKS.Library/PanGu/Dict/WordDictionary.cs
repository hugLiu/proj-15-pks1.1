...
namespace PanGu.Dict
{
    ...
    /// Dictionary for word
    /// </summary>
    public class WordDictionary : AppService, IPerRequestAppService
    {
        ...
        public void Load(String fileName, bool textFile, out string version)
        {
            ...
            var semanticsService = GetService<ISemantics>();
            var dbDict = semanticsService.GetDictionary(null);
            var dbList = new List<WordAttribute>();
            foreach (var item in dbDict)
            {
                dbList.Add(new WordAttribute { Word = item.Term, Pos = (POS)Enum.Parse(typeof(POS), item.Cc) });
            }
            var boService = GetService<IBO2Service>();
            var query = boService.FilterBOs(new FilterRequest());
            foreach (var bo in query)
            {
                dbList.Add(new WordAttribute { Word = bo.BO, Pos = (POS)Enum.Parse(typeof(POS), "BO") });
                foreach (var alia in bo.Alias)
                {
                    dbList.Add(new WordAttribute { Word = alia, Pos = (POS)Enum.Parse(typeof(POS), "BO") });
                }
            }


            //fileName = System.Configuration.ConfigurationSettings.AppSettings["PanGuDict"];
            //// zeyu modify 2017-3-1
            ////fileName = Setting.PanGuSettings.Config.DictionaryApi;
            ////fileName = "http://localhost:1668/API/SemanticsService/GetDic4PanGu";
            //string responseString = string.Empty;
            //WebRequest request = WebRequest.Create(fileName);
            //request.BeginGetResponse(new AsyncCallback(t =>
            //{
            //    using (WebResponse response = request.EndGetResponse(t))
            ...
            //waList = JsonConvert.DeserializeObject<List<WordAttribute>>(responseString, settings);
            waList = dbList;
            ...
            foreach (WordAttribute wa in waList)
            {
                ...
                if (key.Length == 1)
                {
                    ...
                    {
                        _FirstCharDict[key[0]].Pos |= wa.Pos;
                    }
                    continue;
                }
                ...
                if (key.Length == 2)
                {
                    ...
                    {
                        _DoubleCharDict[doubleChar].Pos |= wa.Pos;
                    }
                    continue;
                }
