...
(function (L, window, document, undefined) {
    ...
    L.Control.Position = L.Control.extend({
        ...
        _position : function (event) {
            ...
            try {
                lon =this._input_lon.value;
                //lon = window.magellan(this._input_lon.value).longitude();
                if (lon === null) {
                    throw new Error('Invalid longitude.');
                }
                lon = parseFloat(lon);
                if( (lon.toString() == "NaN") || (lon < -180 || lon > 180))
                {
                    throw new Error('Invalid longitude.');
                }
                lat = this._input_lat.value;
                //lat = window.magellan(this._input_lat.value).latitude();
                if (lat === null) {
                    throw new Error('Invalid latitude.');
                }
                lat = parseFloat(lat);
                if( (lat.toString() == "NaN") ||( lat < -90 || lat >90)){
                    throw new Error('Invalid latitude.');
                }
            } catch (err) {
                alert(err.message);
