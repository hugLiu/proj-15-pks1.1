...
//using PKS.WebAPI.Models.GF;

//namespace PKS.WebAPI.Services
//{
//    public class BOService : IBOService, ISingletonAppService
//    {
//        /// <summary>
...
//            DataTable dt = OracleDBHelper.ExecuteQueryText(strSql.ToString(), parameters).Tables[0];
...
//                List<Alias> list = OracleDBHelper.ExecuteQueryText<Alias>(strSql.ToString(), parameters);
...
//            return OracleDBHelper.ExecuteQueryText<BO>(strSql.ToString(), parameters).FirstOrDefault();
//        }
...
//                    {
//                        for (int j = 0; j < strResult.Split(',')[0].Split(' ').Length; j++)
//                        {
//                            bbox += strResult.Split(',')[0].Split(' ')[j] + ",";
//                        }
//                    }
//                    bbox = bbox.Substring(0, bbox.Length - 1);
//                }
//                strSql.Append(" AND SDO_FILTER(A.GEOMETRY, ");
//                strSql.Append(" MDSYS.SDO_GEOMETRY(2001,  4326, NULL,MDSYS.SDO_ELEM_INFO_ARRAY(1, 1003, 1), ");
...
//            }
//            list.AddRange(OracleDBHelper.ExecuteQueryText<BO>(strSql.ToString()));
//            return list;
//        }
...
//                //父节点
//                treeBOList = OracleDBHelper.ExecuteQueryText<TreeBO>(strSql.ToString(), parameters);
...
//                    strSql.Append(string.Format(" AND T6.BOT = '{0}' ", nodeTemplate.BOT));
//                }
//                List<TreeBO> boList = OracleDBHelper.ExecuteQueryText<TreeBO>(strSql.ToString());
...
//            strSql.Append(" SELECT A.BOID, A.NAME,A.BOT, LTRIM(MAX(SYS_CONNECT_BY_PATH(NAME1, ',')), ',') ALIAS ");
//            strSql.Append(" FROM (SELECT ROW_NUMBER() OVER(PARTITION BY A.BOID, A.NAME,A.BOT ");
//            strSql.Append(" ORDER BY A.BOID, A.NAME,A.BOT) RN,A.* ");
//            strSql.Append(" FROM TAB A) A ");
//            strSql.Append(" START WITH RN = 1 ");
//            strSql.Append(" CONNECT BY PRIOR RN = RN - 1 AND ");
//            strSql.Append(" A.BOID = PRIOR A.BOID ");
//            strSql.Append(" AND  A.NAME = PRIOR A.NAME ");
//            strSql.Append(" GROUP BY a.boid, a.name,a.bot  ");
//            DataTable dt = OracleDBHelper.ExecuteQueryText(strSql.ToString()).Tables[0];
//            foreach (DataRow row in dt.Rows)
//            {
...
//            StringBuilder strSql = new StringBuilder();
//            strSql.Append("SELECT TO_CHAR(SUBSTR(T.GEOMETRY.GET_WKT(),1,3800))  FROM GEOMETRY T WHERE T.BOID=:BOID ");
//            OracleParameter[] parameters = {
//                              new OracleParameter("BOID", OracleDbType.Varchar2,36)
//                                            };
//            parameters[0].Value = boid;
//            string strWKT = OracleDBHelper.ExecuteQueryText<string>(strSql.ToString(), parameters.ToArray()).FirstOrDefault();
//            if (!string.IsNullOrEmpty(strWKT))
//            {
//                list = GetNearBOByLocation(strWKT, distince, bot, filter);
//            }
//            else
//            {
//                list = null;
...
//            string[] arrStr = pointWKT.Split('(');
//            string strResult = System.Text.RegularExpressions.Regex.Replace(pointWKT, @"(.*\()(.*)(\).*)", "$2");
//            if (arrStr[0].Trim().ToUpper() == "POINT" || arrStr[0].Trim().ToUpper() == "LINESTRING")
//            {
//                for (int i = 0; i < strResult.Split(',').Length; i++)
//                {
...
//                    {
//                        strWkt += strResult.Split(',')[i].Split(' ')[j] + ",";
//                    }
//                }
//                strWkt = strWkt.Substring(0, strWkt.Length - 1);
//            }
//            else
//            {
//                strResult = strResult.Substring(0, strResult.Length - 1);
...
//            if (arrStr[0].Trim().ToUpper() == "POINT")//点
//            {
//                strSql.Append(" MDSYS.SDO_GEOMETRY(2001, ");
//                strSql.Append(" 4326, ");
//                strSql.Append(string.Format(" MDSYS.SDO_POINT_TYPE({0},0),", strWkt));
//                strSql.Append(" NULL,NULL),");
//                strSql.Append(" 1) dist ");
//            }
//            else if (arrStr[0].Trim().ToUpper() == "LINESTRING")//线
//            {
//                strSql.Append(" MDSYS.SDO_GEOMETRY(2002, ");
//                strSql.Append(" 4326, ");
//                strSql.Append(" null, ");
//                strSql.Append(" MDSYS.SDO_ELEM_INFO_ARRAY(1,2,1), ");
//                strSql.Append(string.Format(" MDSYS.SDO_ORDINATE_ARRAY({0})  ), ", strWkt));
//                strSql.Append(" 1) dist ");
//            }
//            else //面
//            {
//                strSql.Append(" MDSYS.SDO_GEOMETRY(2003, ");
//                strSql.Append(" 4326, ");
//                strSql.Append(" null, ");
//                strSql.Append(" MDSYS.SDO_ELEM_INFO_ARRAY(1,1003,1), ");
//                strSql.Append(string.Format(" MDSYS.SDO_ORDINATE_ARRAY({0})  ), ", strWkt));
//                strSql.Append(" 1) dist ");
//            }
...
//            }
//            if (!string.IsNullOrEmpty(pointWKT))
//            {
//                if (arrStr[0].Trim().ToUpper() == "POINT")//点
//                {
//                    strSql.Append(" MDSYS.SDO_GEOMETRY(2001, ");
...
//                    strSql.Append(" null, ");
//                    strSql.Append(" MDSYS.SDO_ELEM_INFO_ARRAY(1, 2,  1), ");
//                    strSql.Append(" MDSYS.SDO_ORDINATE_ARRAY({0})  ), ");
//                    strSql.Append(string.Format(" 'distance = {0} unit=M') = 'TRUE' ", strWkt));
//                }
//                else //面
//                {
//                    strSql.Append(" MDSYS.SDO_GEOMETRY(2003, ");
...
//            }
//            list.AddRange(OracleDBHelper.ExecuteQueryText<NearBO>(strSql.ToString()));
//            return list;
//        }
...
//            NearBOCollection list = new NearBOCollection();
//            StringBuilder strSql = new StringBuilder();
//            strSql.Append(" SELECT TO_CHAR(SUBSTR(T3.GEOMETRY.GET_WKT(), 1, 3800)) ");
//            strSql.Append(" FROM BO T ");
//            strSql.Append(" LEFT JOIN OBJECTTYPE T1 ");
//            strSql.Append(" ON T.BOTID = T1.BOTID ");
//            strSql.Append(" LEFT JOIN GEOMETRY T3 ");
//            strSql.Append(" ON T.BOID = T3.BOID ");
...
//                              new OracleParameter("BOT", OracleDbType.Varchar2,50)
//                                            };
//            parameters[0].Value = boName;
//            parameters[1].Value = boType;
//            string strWKT = OracleDBHelper.ExecuteQueryText<string>(strSql.ToString(), parameters.ToArray()).FirstOrDefault();
//            if (!string.IsNullOrEmpty(strWKT))
//            {
//                list = GetNearBOByLocation(strWKT, distince, bot, filter);
...
//                        DataTable enumDt = OracleDBHelper.ExecuteQueryText(strSql.ToString(), parameters).Tables[0];
...
//                        DataTable factDt = OracleDBHelper.ExecuteQueryText(strSql.ToString(), factParameters).Tables[0];
...
//            return OracleDBHelper.ExecuteQueryText<DbInfo>(strSql.ToString()).FirstOrDefault();
//        }
...
//            List<Models.AppDomain> list = new List<Models.AppDomain>();
//            StringBuilder strSql = new StringBuilder();
//            strSql.Append(" WITH TAB AS( ");
...
//            foreach (DataRow row in dt.Rows)
//            {
//                var appdomain = new Models.AppDomain();
