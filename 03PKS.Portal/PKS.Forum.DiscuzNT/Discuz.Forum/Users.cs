...
namespace Discuz.Forum
{
    ...
    public class Users
    {
        ...
        {
            return Discuz.Data.Users.GetDefaultAdminId();
        }
        ...
        {
            DNTCache cache = DNTCache.GetCacheService();
            string[] values = cache.RetrieveObject("/Forum/AdminUsers") as string[];
            if (values == null)
            {
                var admins = Data.Users.GetUserListByGroupid("1,2,3");
                values = admins.Rows.Cast<DataRow>().Select(e => e["username"].ToString()).ToArray();
                cache.AddObject("/Forum/AdminUsers", values);
            }
            return values;
        }
