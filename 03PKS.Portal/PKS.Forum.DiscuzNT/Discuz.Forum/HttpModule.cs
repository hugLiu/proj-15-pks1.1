...
namespace Discuz.Forum
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        public void Init(HttpApplication context)
        {
            ...
            //context.AuthenticateRequest += Discuz_AuthenticateRequest;
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            //Debug.WriteLine(context.Request.Url.ToString());
            string forumPath = baseconfig.Forumpath.ToLower();

            string requestPath = context.Request.Path.ToLower();
            if (requestPath.StartsWith("/webresource.axd")) return;
            if (requestPath.StartsWith(forumPath + "cache/")) return;
            if (requestPath.StartsWith(forumPath + "editor/")) return;
            if (requestPath.StartsWith(forumPath + "images/")) return;
            if (requestPath.StartsWith(forumPath + "javascript/")) return;
            if (requestPath.StartsWith(forumPath + "redirect/")) return;
            if (requestPath.StartsWith(forumPath + "commoninfo/")) return;
            if (requestPath.StartsWith(forumPath + "ssoaccount/")) return;

            context.Items["PKS.Context.RawRequestUrl"] = context.Request.Url.ToString();
            //使用版块
            ...
            if (requestPath.StartsWith(forumPath))
            {
                if (requestPath.Substring(forumPath.Length).IndexOf("/") == -1)
                {
                    ...
                    if (requestPath == forumPath || requestPath.EndsWith("/index.aspx"))
                    {
        ...
        /// 第二步：验证用户身份
        /// </summary>
        private void Discuz_AuthenticateRequest(object sender, EventArgs e)
        {
